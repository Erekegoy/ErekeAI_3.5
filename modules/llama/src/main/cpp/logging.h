...
#define LOGv(...) \
    do { \
        if (ai_should_log(ANDROID_LOG_VERBOSE)) \
...
#define LOGd(...) \
    do { \
        if (ai_should_log(ANDROID_LOG_DEBUG)) \
...
#define LOGi(...) \
    do { \
        if (ai_should_log(ANDROID_LOG_INFO)) \
            __android_log_print(ANDROID_LOG_INFO, LOG_TAG, __VA_ARGS__); \
    } while (0)

#define LOGw(...) \
    do { \
        if (ai_should_log(ANDROID_LOG_WARN)) \
            __android_log_print(ANDROID_LOG_WARN, LOG_TAG, __VA_ARGS__); \
    } while (0)

#define LOGe(...) \
    do { \
        if (ai_should_log(ANDROID_LOG_ERROR)) \
